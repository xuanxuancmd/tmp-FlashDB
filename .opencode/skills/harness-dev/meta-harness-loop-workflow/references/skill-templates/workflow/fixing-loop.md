...
Fixing 由**主 Agent** 编排：拉起 executor(mode=fix) 修复 → 拉起 checker sub-agent 重新验证。本文件是修复循环的通用权威定义，workflow 的 review/evaluate stage 均引用本文件，仅指定实例参数（trigger_stage / checker 类型 / worktree_path）。
...
│      ↓                                                  │
│  ② 【修前】加载 fix-self-check skill（若 {fixing_skill}  │
...
│      ↓                                                  │
│  ④ 【修后】加载 fix-self-check skill（若 {fixing_skill}  │
...
完整路径：
```
review 失败 → on_failure: fix → 执行 fix →
  ├─ fix 成功 → 回 trigger_stage(review) 重新检测
  │   ├─ review 通过 → 进入下一 stage
  │   └─ review 又失败 → round++ → 再触发 fix（直到 round >= max_rounds）
...
5. **加载 fix-self-check skill（若 {fixing_skill} 非空）**：主 Agent 在决策修复策略时加载，执行修前检查（因果链+爆炸半径）+ 修后检查（回归+趋势）
