...
4. pass=false → 按需Read evidence文件（原则2）→ Read report.md（原则3）→ 提取 blocking_issues
5. 拉起 executor(mode=fix) sub-agent，传入 issues + SUMMARY 路径 → 等待返回
6. 修复后重新检视（retry += 1）
7. 达到3次仍未通过 → 向用户报告失败，请求介入
