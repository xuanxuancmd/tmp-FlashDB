...
| 技能 | 用途 |
|------|------|
| fix-self-check | 修复环节强制自检，修前验证方向（因果链+爆炸半径）、修后验证健康（回归+趋势） |

> 若上表为空，则 Fixing 阶段不加载自检技能，直接进入修复循环。
...
│      ↓                                                  │
│  ② 【修前】加载 fix-self-check skill（若上表非空），      │
│     执行修前检查（因果链诊断 + 爆炸半径论证）              │
│      ↓                                                  │
...
│      ↓                                                  │
│  ④ 【修后】加载 fix-self-check skill（若上表非空），      │
│     执行修后检查（回归检测 + diff 质量）                  │
│      ↓                                                  │
...
5. **加载 fix-self-check skill（若上表非空）**：主 Agent 在决策修复策略时加载，执行修前检查（因果链+爆炸半径）+ 修后检查（回归+趋势）
