...
+ **1:1 翻译约束**：代码翻译保证目录、文件数、结构体、函数名 1:1 对齐（采用 Rust snake_case 命名风格）；语言差异过大无法 1:1 映射时，需重点分析迁移方案并提交用户审阅
+ **目录映射**：C 源码路径保持相对路径。如 `src/fdb_kvdb.c` → `src/fdb_kvdb.rs`，`inc/fdb_def.h` → `inc/fdb_def.rs`
+ **文件映射**：C 独立 `.c`/`.h` 文件 → Rust 独立 `.rs` 文件，不能合并多个 struct 到同一文件
