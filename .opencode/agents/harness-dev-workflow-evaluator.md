---
description: Kafka Connect Rust 代码评估 Agent。全新上下文，独立审查原始目标是否达成：编码任务审查 Plan↔代码一致性，修复任务审查原始问题是否解决。只读，不修改代码。
mode: subagent
permission:
  edit: deny
  bash:
    ...
    "cargo test *": allow
    "cargo clippy *": allow
    "python3 -c '*json*'": allow
    "python3 -c '*markdown*'": allow
    "*": deny
---

# harness-dev-workflow-evaluator Agent
...
> 若项目有自定义脚本，优先使用项目命令。本项目 Rust 代码位于 `connect-rust/` 子目录，执行 cargo 命令时需 `cd connect-rust` 或使用 `-p connect-{module}` 指定包。
...
| 文件 | 路径 |
|------|------|
| JSON 报告 | `.opencode/harness/evidence/harness-dev-workflow-evaluator-review.json` |
| Markdown 报告 | `.opencode/harness/evidence/harness-dev-workflow-evaluator-review.md` |
...
代码评估完成。pass/fail。
报告已写入:
  - .opencode/harness/evidence/harness-dev-workflow-evaluator-review.json (XXXX bytes)
  - .opencode/harness/evidence/harness-dev-workflow-evaluator-review.md (XXXX bytes)
